...
func createSession(conn *websocket.Conn, code string, config Config, onClose func(session *Session)) *Session {
	ctx, cancel := context.WithCancel(logs.ContextWithTrace(context.Background()))
...
func (s *Session) Close() {
	...
	s.closeOnce.Do(func() {
		...
		logs.Info(s.ctx, "websocket session closed", map[string]any{"code": s.code})
	})
}
...
func (s *Session) replaceConn(newConn *websocket.Conn) {
	...
	logs.Info(s.ctx, "websocket connection replaced", map[string]any{"code": s.code})
}
...
func (s *Session) readLoop(conn *websocket.Conn, seq uint64) {
	defer func() {
		if value := recover(); value != nil {
			logs.Error(s.ctx, "websocket readLoop panic", map[string]any{
				"code":  s.code,
				"panic": value,
		...
		logs.Info(s.ctx, "websocket readLoop exited", map[string]any{"code": s.code, "seq": seq})
	}()
	...
	for {
		...
		if err != nil {
			if s.isCurrentConn(conn, seq) {
				logs.Error(s.ctx, "websocket read message failed", map[string]any{"code": s.code}, err)
				s.Close()
			}
			return
		}
		...
		case s.readCh <- data:
		default:
			logs.Warn(s.ctx, "websocket read channel full", map[string]any{"code": s.code, "len": len(data)}, nil)
		}
	}
}
...
func (s *Session) writeLoop() {
	defer func() {
		if value := recover(); value != nil {
			logs.Error(s.ctx, "websocket writeLoop panic", map[string]any{
				"code":  s.code,
				"panic": value,
				"stack": string(debug.Stack()),
			}, nil)
		...
		logs.Info(s.ctx, "websocket writeLoop exited", map[string]any{"code": s.code})
	}()
	...
	for {
		...
		case msg := <-s.writeCh:
			...
			if err := conn.WriteMessage(s.messageType, msg); err != nil {
				logs.Error(s.ctx, "websocket write message failed", map[string]any{"code": s.code, "len": len(msg)}, err)
				if s.isCurrentConn(conn, seq) {
					s.Close()
				}
			}
