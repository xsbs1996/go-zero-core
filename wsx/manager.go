...
func (m *Manager) Create(w http.ResponseWriter, r *http.Request, code string) (*Session, bool, error) {
	reqCtx := logs.ContextWithTrace(r.Context())
	code = strings.TrimSpace(code)

	if code == "" {
		logs.Error(reqCtx, "invalid websocket code", map[string]any{"addr": r.RemoteAddr}, ErrInvalidCode)
		return nil, false, ErrInvalidCode
	}

	conn, err := m.upgrader.Upgrade(w, r, nil)
	if err != nil {
		logs.Error(reqCtx, "websocket upgrade failed", map[string]any{"addr": r.RemoteAddr, "code": code}, err)
	...
	if oldSession, exists := m.sessions[code]; exists {
		...
		logs.Info(reqCtx, "websocket session reconnected", map[string]any{"code": code})
	...
	if m.total.Load() >= m.config.MaxConnTotal {
		m.mu.Unlock()
		_ = conn.Close()
		logs.Warn(reqCtx, "websocket max connection exceeded", map[string]any{"code": code, "total": m.total.Load()}, ErrMaxConnExceeded)
	...
	logs.Info(reqCtx, "websocket session created", map[string]any{"code": code, "total": total})
