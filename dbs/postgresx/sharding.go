...
var (
	ErrInvalidPrimaryKeyGenerator     = errors.New("postgresx: invalid sharding primary key generator") // ErrInvalidPrimaryKeyGenerator 表示分表主键生成策略非法，当前支持 snowflake、custom、none。
	ErrShardingPrepareStmtUnsupported = errors.New("postgresx: sharding does not support prepare stmt") // ErrShardingPrepareStmtUnsupported 表示分表插件不支持 GORM PrepareStmt。
	ErrMissingShards                  = errors.New("postgresx: missing sharding shards")                // ErrMissingShards 表示分表数量为空。
	ErrMissingTables                  = errors.New("postgresx: missing sharding tables")                // ErrMissingTables 表示分表表名映射为空。
	ErrMissingKey                     = errors.New("postgresx: missing sharding key")                   // ErrMissingKey 表示分表键为空。
)
