...
var (
	ErrAlreadyInitialized = errors.New("postgresx: database already initialized") // ErrAlreadyInitialized 表示全局 GORM 单例已经初始化。
	ErrNotInitialized     = errors.New("postgresx: database not initialized")     // ErrNotInitialized 表示全局 GORM 单例未初始化。
)
...
func GetDB() *gorm.DB {
	...
	if gormDB == nil {
		panic("postgresx: database not initialized, call postgresx.Init or postgresx.SetDB first")
	}
	return gormDB
}
