...
var (
	ErrInvalidPrimaryKeyGenerator     = errors.New("mysqlx: invalid sharding primary key generator") // ErrInvalidPrimaryKeyGenerator 表示分表主键生成策略非法，当前支持 snowflake、custom、none。
	ErrShardingPrepareStmtUnsupported = errors.New("mysqlx: sharding does not support prepare stmt") // ErrShardingPrepareStmtUnsupported 表示分表插件不支持 GORM PrepareStmt。
	ErrMissingShards                  = errors.New("mysqlx: missing sharding shards")                // ErrMissingShards 表示分表数量为空。
	ErrMissingTables                  = errors.New("mysqlx: missing sharding tables")                // ErrMissingTables 表示分表表名映射为空。
	ErrMissingKey                     = errors.New("mysqlx: missing sharding key")                   // ErrMissingKey 表示分表键为空。
)
...
func useSharding(db *gorm.DB, conf Sharding, opts connectOptions) error {
	...
	for tableName, tableConf := range conf.Tables {
		...
		if err := db.Use(gormsharding.Register(gormsharding.Config{
			...
			return fmt.Errorf("register mysql sharding failed: %w", err)
		}
	}
