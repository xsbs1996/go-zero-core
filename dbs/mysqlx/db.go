...
var (
	ErrAlreadyInitialized = errors.New("mysqlx: database already initialized") // ErrAlreadyInitialized 表示全局 GORM 单例已经初始化。
	ErrNotInitialized     = errors.New("mysqlx: database not initialized")     // ErrNotInitialized 表示全局 GORM 单例未初始化。
)
...
func GetDB() *gorm.DB {
	...
	if gormDB == nil {
		panic("mysqlx: database not initialized, call mysqlx.Init or mysqlx.SetDB first")
	}
	return gormDB
}
