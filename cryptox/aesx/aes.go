...
var (
	ErrInvalidKey = errors.New("aesx: invalid key size") // ErrInvalidKey 表示 AES 密钥长度不合法。
	ErrInvalidIV  = errors.New("aesx: invalid iv size")  // ErrInvalidIV 表示 CBC 初始向量长度不合法。
)
...
func DecryptGCM(cipherText string, key []byte) ([]byte, error) {
	...
	if len(data) < gcm.NonceSize() {
		return nil, errors.New("aesx: ciphertext too short")
	}
...
func DecryptCBC(cipherText string, key []byte, iv []byte) ([]byte, error) {
	...
	if len(data) == 0 || len(data)%block.BlockSize() != 0 {
		return nil, errors.New("aesx: invalid ciphertext size")
	}
...
func pkcs7UnPadding(data []byte, blockSize int) ([]byte, error) {
	if len(data) == 0 || len(data)%blockSize != 0 {
		return nil, errors.New("aesx: invalid padding size")
	}

	padding := int(data[len(data)-1])
	if padding == 0 || padding > blockSize || padding > len(data) {
		return nil, errors.New("aesx: invalid padding")
	}
